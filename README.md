...
Movement accelerates the longer you hold: starts at 100 px/s and ramps up to 2400 px/s. These values are all modifiable in `src/config.rs`.
...
- **macOS note:** ViMouse requires Accessibility permission to intercept input. On first launch, ViMouse will prompt you to grant it under **System Settings → Privacy & Security → Accessibility**.
...
The binary is located at `target/release/vimouse.exe`. Run it directly or add it to your startup folder.

> [!NOTE]
> Known Limitations on Windows:
> - Unless ViMouse is launched with administrator privileges, it cannot interact with admin-level processes, such as Task Manager, UAC, Admin Command Prompt, etc.
